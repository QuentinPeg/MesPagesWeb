...
const Header: React.FC = () => {
  ...
  return (
    <div>
      <nav className='bg-blue-500 text-white p-4 flex justify-between items-center max-sm:hidden'>{/*NAVIGATION Oridnateur*/}
      ...
      <nav className='bg-blue-500 text-white p-4 flex justify-between items-center sm:hidden'>{/*NAVIGATION Mobile*/}
        <Link to={user ? "/accountform" : "/"} className="flex items-center text-white mr-4">
          <img src={bankingImage} alt="Logo" className="w-12 rounded-full" style={{ cursor: 'pointer' }} />
          <h1 className="text-2xl pl-3">Gestion de Comptes</h1>
        </Link>
        {user ? (
          <div className='flex justify-evenly sm:flex-row flex-col w-max items-center'>
            ...
            <div className={`flex sm:w-full sm:flex-row flex-col justify-evenly  items-center sm:static sm:top-auto max-sm:absolute max-sm:top-[100px] max-sm:left-1/2 transform max-sm:-translate-x-1/2 bg-blue-500 p-4 rounded-md w-[100vw] z-10 ${isVisible ? 'block' : 'hidden'} sm:block`}>

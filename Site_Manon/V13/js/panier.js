...
function supprimerArticle(event, boutonSupprimer) {
    ...
    if (elementASupprimer) {
        var nomArticle = elementASupprimer.firstChild.textContent.replace(/\[|\]/g, "").replace(/[0-9]/g, "").replace(/x/g, "").trim(); // Récupérer le nom de l'article
        ...
        for (var i = 0; i < elementsArticlesPrecommande.length; i++) {
            ...
            var idArticle = articlePrecommande.id.replace(/\[|\]/g, "").replace(/[0-9]/g, "").replace(/x/g, "").trim();

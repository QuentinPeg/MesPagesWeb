...
function afficherRecapArticles() {
    ...
    for (var i = 0; i < panier.length; i++) {
        ...
        var key = `${nom}-${article.format}-${article.plastifie}`;
        ...
        var selectFormat = createFormatSelect(nom, quantite, key, article.format);

        var labelPlastifie = document.createElement('label');
        labelPlastifie.textContent = 'Plastifié :';
        var selectPlastifie = createPlastifieSelect(nom, quantite, key, article.plastifie);
...
function createPlastifieSelect(nom, quantite, key, selectedPlastifie) {
    ...
    selectPlastifie.addEventListener('change', function (event) {
        ...
        var articleIndex = panier.findIndex(a => `${a.nom}-${a.format}-${a.plastifie}` === key);
        if (articleIndex !== -1) {
            ...
            if (selectedPlastifie === 'Non') {
                var selectFormat = document.querySelector(`select[data-nom="${nom}"][data-quantite="${quantite}"][data-key="${key}"]`);
                if (selectFormat) {
